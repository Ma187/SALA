...
def parse_args(comstr=None):
    ...
    parser.add_argument('--outfile', type=str, default='SALA.csv', help='name of output file')
    ...
    # parser.add_argument('--_batch_size', type=int, default=2)
    ...
    parser.add_argument('--forward_type', default='None', type=str,
                        choices=['None', 'perturb', 'mask', 'pm', 'mm', 'pp', 'mp', 'np','nm'])
    parser.add_argument('--only_max_min_noise', default='None', type=str,
                        choices=['None', 'max_min', 'max', 'near_max_min'])
    parser.add_argument('--only_max_min_mask', default='None', type=str,
                        choices=['None', 'max_min', 'max', 'near_max_min'])
...
def main(args, dataset_name=None):
    ...
    if 'synthetic' in args.dataset:
        path = os.path.abspath(os.path.join(args.basicpath, 'src', 'ucr_data', 'data_files', args.dataset))
        X = np.load(os.path.join(path,'X_data.npy'))
        Y = np.load(os.path.join(path,'Y_data.npy'))
    elif args.dataset in datasets.ucr_dataset_list():
    ...
    for kk, (trn_index, test_index) in enumerate(skf.split(X, Y)):
        ...
        if args.use_wandb:
            wandb.init(
                ...
                name=args.group + f"_Seed{seeds[kk]}",
        ...
        if args.valid_set:
            ...
            trn_index, val_index = next(iter(train_valid_skf.split(x_train, Y_train_clean)))
        ...
        if args.model in ['vanilla']:
            ...
            df_results = main_wrapper_sanm(args, x_train, x_test, Y_train_clean, Y_test_clean, saver,
                                           x_valid=x_valid, y_valid_clean=y_valid_clean, seed=seeds[seeds_i])
        else: # args.model in ['SALA']:
...
def experiment(args):
    ...
    if args.dataset == '':
        for dataset_name in ucr:
            ...
            # pd.DataFrame(result_value).to_csv(path, mode='a', header=False)
