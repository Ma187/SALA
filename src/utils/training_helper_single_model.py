...
from src.utils.utils_ import readable, reset_seed_, reset_model, flip_label, remove_empty_dirs, \
    ...
    TruncatedLoss,save_model_and_sel_dict
...
def train_step(data_loader, model,model2=None):
    model = model.eval()
    if model2 is not None:
        model2 = model2.eval()
    total_num = len(data_loader.dataset.tensors[1])
    ...
    for x, y_noisy, _, y in data_loader:
        x = x.to(device)
        ...
        try:
            ...
            y = y.numpy()
            y_noisy = y_noisy.numpy()
        ytrue.append(y)
        noisy_labels.append(y_noisy)
    ...
    y_hat_labels = np.argmax(y_hat_proba, axis=1)
    accuracy = accuracy_score(ytrue, y_hat_labels)
    noisy_accuracy = accuracy_score(noisy_labels, y_hat_labels)
    f1_weighted = f1_score(ytrue, y_hat_labels, average='weighted')

    return accuracy, f1_weighted,noisy_accuracy
...
def train_model(model, train_loader, test_loader, args, tau,train_dataset=None,saver=None,valid_loader=None):
    criterion = nn.CrossEntropyLoss(reduce=False)
    optimizer = optim.Adam(model.parameters(), lr=args.lr, eps=1e-4)
    # learning history
    train_acc_list = []
    clean_train_acc_list = []
    noisy_train_acc_list = []
    train_acc_list_aug = []
    train_avg_loss_list = []
    test_acc_list = []
    test_f1s = []
    ...
    try:
        ...
        for e in range(args.epochs):
            ...
            # testing
            clean_train_acc, clean_f1, noisy_train_acc = train_step(data_loader=train_loader,
                                          model=model)
            ...
            wandb.log({'Train Acc': train_accuracy, 'Test Acc': test_accuracy, 'Test f1': f1}, step=e)
            ...
            clean_train_acc_list.append(clean_train_acc)
            noisy_train_acc_list.append(noisy_train_acc)
            test_f1s.append(f1)
        ...
        os.makedirs('./other_accs/',exist_ok=True)
        pd.DataFrame(test_acc_list, columns=[args.seed]).T.to_csv(
            './other_accs/{}_{}_test_acc_list.csv'.format(args.dataset, args.outfile.split('.')[0]),
            mode='a', header=0)
    ...
    test_results_last_ten_epochs['last_ten_clean_train_acc'] = clean_train_acc_list[-10:]
    test_results_last_ten_epochs['last_ten_noisy_train_acc'] = noisy_train_acc_list[-10:]
...
def train_eval_model(model, x_train, x_test, Y_train, Y_test, Y_train_clean,
    ...
    model, test_results_last_ten_epochs = train_model(model, train_loader, test_loader, args, ni,
                                          train_dataset=train_dataset,saver=saver,valid_loader=valid_loader)
    print('Train ended')

    ########################################## Eval ############################################

    # save test_results: test_acc(the final model), test_f1(the final model), avg_last_ten_test_acc, avg_last_ten_test_f1
    test_results = evaluate_class(model, args, x_test, Y_test, None, test_loader, ni, saver, 'CNN',
    ...
    test_results['noisy_train_acc'] = test_results_last_ten_epochs['last_ten_noisy_train_acc'][-1]
    test_results['clean_train_acc'] = test_results_last_ten_epochs['last_ten_clean_train_acc'][-1]
    ...
    if plt_embedding and args.embedding_size <= 3:
        plot_embedding(args, model.encoder, train_eval_loader, Y_train_clean,
...
def main_wrapper_single_model(args, x_train, x_test, Y_train_clean, Y_test_clean, saver,seed=None,
                              x_valid=None,y_valid_clean=None):
...
def warmup_single_model(data_loader, model, optimizer, criterion,epoch=None,
    ...
    for batch_idx,(x, y_hat,x_idx,mask_train) in enumerate(data_loader):
        ...
        if hasattr(model,'decoder'):
            ...
            out, hd, h = model(x, recon=True, encoder=True)
            out = out['bag_logits']
            model_loss = criterion(out, y_hat)
            loss_all[x_idx, epoch] = model_loss.data.detach().clone().cpu().numpy()
            model_loss = model_loss.sum()+args.L_rec_coef*nn.MSELoss(reduction='mean')(hd,x)
        else:
            out = model(x)
            print('out: {} \n y_hat: {}'.format(out,y_hat))
...
def train_ELR(data_loader, model, optimizer, criterion,epoch=None,
                         loss_all=None,args=None):
    global_step = 0
    avg_accuracy = 0.
    avg_loss1 = 0.
    ...
    for batch_idx,(x, y_hat,x_idx,mask_train) in enumerate(data_loader):
        ...
        if hasattr(model,'decoder'):
            ...
            hd=model.decoder(h)
            out=model.classifier(h.squeeze(-1))
            model_loss = criterion(x_idx,out, y_hat)
            loss_all[x_idx, epoch] = model_loss.data.detach().clone().cpu().numpy()
            model_loss = model_loss+nn.MSELoss(reduction='mean')(hd,x)
        else:
            out = model(x)
            model_loss = criterion(x_idx,out, y_hat)
