...
def build_dataset_pt(args):
    ...
    if len(train_dataset.shape) == 2:
        return train_dataset[:,None,:], train_target, test_dataset[:,None,:], test_target, num_classes, val_dataset[:,None,:], val_target
    else:
        return train_dataset, train_target, test_dataset, test_target, num_classes, val_dataset, val_target


# def build_dataset_pt(args):
#     dat a_path = f"../data/ts_noise_data/{args.dataset}"
#     train_dataset_dict = torch.load(os.path.join(data_path, "train.pt"))
#     train_dataset = train_dataset_dict["samples"].numpy()  # (num_size, num_dimensions, series_length)
#     train_target = train_dataset_dict["labels"].numpy()
#     num_classes = len(np.unique(train_dataset_dict["labels"].numpy(), return_counts=True)[0])
...
def evaluate_scale_flow_acc(val_loader, model_list, classifier_list, loss_list, scale_list, device):
    ...
    for data, target in val_loader:
        with torch.no_grad():
            ...
            for scale_value in scale_list:
                if index_s == 0:
                    val_pred, pred_embed = model_list[index_s](downsample_torch(data, sample_rate=scale_value, device=device), encoder=True)
                    up_scale_embed = pred_embed

                    if index_s == (len(scale_list) - 1):  ## len(scale_list) = 1, this is a single scale
                        val_pred = val_pred['bag_logits']
                    ...
                    val_pred, pred_embed = model_list[index_s](downsample_torch(data, sample_rate=scale_value, device=device), encoder=True, scale=scale_value)
                    up_scale_embed = pred_embed
                    val_pred = val_pred['bag_logits'].to(device)
    ...
    return val_loss / num_val_samples, accuracy_score(target_true, target_pred), f1_score(target_true, target_pred, average='weighted')
