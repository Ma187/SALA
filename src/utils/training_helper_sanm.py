...
from src.models.MILLET_model import (
    ...
    MILAttentionPooling,
    MILAdditivePooling)
from src.models.MILLET_model import MILLET
from src.models.MILLET_sanm import MILLET as MILLET_sanm
...
def train_model(model, train_loader, test_loader, args, train_dataset=None, saver=None, valid_loader=None):
        ...
        for e in range(args.epochs):
            ...
            print(
                '{} epoch - Train Loss {:.4f}\tTrain accuracy {:.4f}\tTest accuracy {:.4f}'.format(
                    ...
                    train_acc_oir,
                    test_accuracy))
            if args.save_model and e % 10 == 0 and args.seed == 37:
    ...
    if args.save_model and args.seed == 37:
...
def main_wrapper_global_sel_model(args, x_train, x_test, Y_train_clean, Y_test_clean, saver, seed=None, x_valid=None,
    ...
    if args.MILLET:
        ...
        if args.backbone == 'MLP':
            ...
            backbone = MILLET_sanm if args.patch else MILLET
            model = backbone(
...
def warmup_global_sel_model(data_loader, model, optimizer, criterion, epoch=None,
    ...
    for batch_idx, (x, y_hat, x_idx, _) in enumerate(data_loader):
        ...
        if args.MILLET:
            if args.recon:
                out_dict, x_recon = model(x, recon=True)
                if args.backbone == 'MLP':
...
def train_step_global_sel_model(data_loader, model, optimizer, criterion, loss_all=None, epoch=0, args=None,
                                scaler=None, confident_id=None, LshShapelets=None, pred_label_weight=0.8,
                                conf_prob=None):
    global_step = 0
    aug_step = 0
    avg_accuracy = 0.
    ...
    conf_prob = torch.from_numpy(conf_prob).view(-1)
    pseudo_error = []
    pseudo_acc = []
    MseLoss = nn.MSELoss()
    ...
    for batch_idx, (x, y_hat, x_idx, y_clean) in enumerate(data_loader):
        ...
        if args.MILLET:
            ...
            if args.recon:
                ...
                out_dict = model(x)
            model_loss = criterion(out_dict['bag_logits'], y_hat)
            pred_softlabel = torch.softmax(out_dict['bag_logits'], dim=1)
            model.interpre[x_idx] = out_dict[args.interpre_type].clone().detach().cpu()
        else:
            out = model(x)
            pred_softlabel = torch.softmax(out, dim=1)
            model_loss = criterion(out, y_hat)
        ...
        clean_sample_indices = torch.where(clean_sample_mask)[0]
        model_loss = model_loss[clean_sample_indices].sum()
        pseudo_loss = 0.
        if args.label_correct_type == 'soft':
            noisy_pred_softlabel = pred_softlabel[~clean_sample_mask]
            pred_pseudo = probs * F.one_hot(y_hat, num_classes=args.nbins).float()[~clean_sample_mask] + \
                          (1 - probs) * noisy_pred_softlabel
            with torch.no_grad():
                if args.pseudo_label == 'pred':
                    pseudo_label = pred_pseudo
                else:  # 'pred_shapelets'
                    uns_softlabel = LshShapelets.getSoftLabelsByShapelets(x[~clean_sample_mask], args.temperature).to(
                        device)
                    if args.soft_weight:
                        pseudo_label = combinePseudo(pred_pseudo, uns_softlabel)
                    else:
                        pseudo_label = pred_label_weight * pred_pseudo + (1 - pred_label_weight) * uns_softlabel
                max_probs, _ = torch.max(pseudo_label, dim=1)
                selected_indices = max_probs > args.correct_threshold
                selected_pseudo_labels = pseudo_label[selected_indices]
            selected_pred_softlabel = noisy_pred_softlabel[selected_indices]
            if len(selected_pseudo_labels) != 0:
                pseudo_error += calculate_mse_loss(selected_pseudo_labels,
                                                   y_clean.to(device)[~clean_sample_mask][selected_indices],
                                                   args.nbins)
                pseudo_acc += calculate_label_acc(selected_pseudo_labels,
                                                  y_clean.to(device)[~clean_sample_mask][selected_indices])
                pseudo_loss = MseLoss(selected_pred_softlabel, selected_pseudo_labels)


        model_loss = model_loss + args.L_rec_coef * recon_loss + args.pseudo_loss_coef * pseudo_loss
        ...
        # Compute accuracy
        if args.MILLET:
            acc1 = torch.eq(torch.argmax(out_dict['bag_logits'], 1), y).float()
        else:
...
def train_step(data_loader, model, optimizer, criterion, loss_all=None, epoch=0, args=None,
    ...
    for batch_idx, (x, y_hat, x_idx, y_clean) in enumerate(data_loader):
        ...
        model_args = {'bags': x, 'pos': None, 'recon': args.recon, 'conf_mask': None, 'label': y_hat,
                      'forward_type': args.forward_type, 'conf_score': conf_prob, 'x_idx': x_idx,
                      'only_max_min_noise':args.only_max_min_noise,
                      'only_max_min_mask':args.only_max_min_mask} 
        if args.forward_type in ['pm', 'pp']:  
            model_args['forward_type'] = 'perturb'
        elif args.forward_type in ['mm','mp']:  
            model_args['forward_type'] = 'mask'

        if args.recon:
            out_dict, x_recon = model(**model_args)
            if args.forward_type in ['pm','mm']:   
                model_args['forward_type'] = 'mask'
                out_dict2, _ = model(**model_args)
            elif args.forward_type in ['pp','mp']:   
                model_args['forward_type'] = 'perturb'
                out_dict2, _ = model(**model_args)
            if args.backbone == 'MLP':
                recon_loss = MseLoss(x.unfold(dimension=-1, size=args.patch_len,
        ...
        if args.forward_type in ['pm','mm','pp','mp']:
