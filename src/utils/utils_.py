...
def build_dataset_pt(args):
    ...
    if args.dataset == 'FD_A':
        args.sample_len = train_dataset.shape[1]
        args.nvars = 1
    else:
        args.sample_len = train_dataset.shape[2]
        args.nvars = train_dataset.shape[1]
    if not args.MILLET:
        train_dataset = train_dataset.transpose(0, 2, 1)  # (num_size, series_length, num_dimensions)
        args.sample_len = train_dataset.shape[1]
        args.nvars = train_dataset.shape[2]
    ...
    return train_dataset, train_target, test_dataset, test_target, num_classes, val_dataset, val_target
...
def evaluate_model(model, dataloder, y_true, args):
    if isinstance(model,list) and model[1] is not None:
        yhat1 = predict(model[0], dataloder, args)
        yhat2 = predict(model[1], dataloder, args)
        yhat = (yhat1+yhat2)/2
    elif isinstance(model,list):
        yhat = predict(model[0], dataloder, args)
    else:
        yhat = predict(model, dataloder, args)
...
def evaluate_class(model, args, x, Y, Y_clean, dataloader, ni, saver, network='Model', datatype='Train', correct=False,
    ...
    results, yhat_proba, yhat, acc, f1 = evaluate_model(model, dataloader, Y, args)
...
def predict(model, test_data, args = None):
    prediction = []
    with torch.no_grad():
        model.eval()
        for data in test_data:
            ...
            try:
                ...
                prediction.append(out_dict[0].cpu().numpy())
...
def predict_multi(model, test_data):
    ...
    with torch.no_grad():
        ...
        for data in test_data:
            ...
            out_class, out_ae, embedding = model(data, recon=True, encoder=True)
            out_class = out_class['bag_logits']
...
def sigua_loss(model_loss, rt, bad_weight,last_sel_id=None,current_batch_idx=None,args=None):
    ...
    return model_loss,current_batch_idx[good_data_idx.cpu()].cpu().numpy()
...
def train_step(model, train_loader,args,loss_all,pt_model=None,epoch=0):
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (x, y_hat, x_idx, _) in enumerate(train_loader):
            ...
            if args.MILLET:
                if args.mode == 'smooth':
                    out_dict = model(x,mode=args.mode, x_idx=x_idx)
                else:
                    out_dict = model(x)
                ...
                if pt_model is not None:
                    pt_out_dict = pt_model(x)
                    noisy_interpre = pt_out_dict[args.interpre_type].clone().detach()

                    noisy_interpre = torch.softmax(noisy_interpre, dim=1)
                    model.interpre[x_idx] = torch.softmax(model.interpre[x_idx], dim=1)
                    labels = y_hat[:, None, None].expand(-1, -1, noisy_interpre.shape[2])
                    noisy_interpre = torch.gather(noisy_interpre, 1, labels)
                    model.interpre[x_idx] = noisy_interpre.cpu() - model.interpre[x_idx]
...
def select_class_by_class_with_GMM(loss_all=None, args=None, labels=None, p_threshold=0.5,
                                   model=None, train_loader=None, pt_model=None):
    '''
    Select confident samples class by class using GMM.
    '''
    if model is not None:
        loss_all = train_step(model, train_loader,args,loss_all, pt_model=pt_model)
    labels_numpy = labels.detach().cpu().numpy()
...
def select_with_GMM(loss_all=None, args=None, labels=None, p_threshold=0.5,model=None,
                    train_loader=None,pt_model=None):
    '''
    Select confident samples using GMM for binary classification.
    '''

    if model is not None:
        loss_all = train_step(model, train_loader,args,loss_all,pt_model)
...
def calculate_mse_loss(pseudo_label, y_clean, num_classes):

    with torch.no_grad():
        y_clean_one_hot = F.one_hot(y_clean, num_classes=num_classes).float()

        individual_losses = ((pseudo_label - y_clean_one_hot) ** 2).mean(dim=1)

    loss_list = individual_losses.tolist()
    return loss_list


def calculate_label_acc(pseudo_label, y_clean):
    with torch.no_grad():
        pred_label = torch.argmax(pseudo_label,dim=1)
        return torch.eq(pred_label, y_clean).float().tolist()


def combinePseudo(pred_pseudo, shapelet_pseudo):
    epsilon = 1e-3
    pred_pseudo = pred_pseudo.clamp(min=epsilon)
    shapelet_pseudo = shapelet_pseudo.clamp(min=epsilon)
    entropy1 = -torch.sum(pred_pseudo * torch.log(pred_pseudo), dim=1)
    entropy2 = -torch.sum(shapelet_pseudo * torch.log(shapelet_pseudo), dim=1)
    w1 = (entropy2 / (entropy1 + entropy2)).view(-1,1)
    w2 = 1 - w1
    pseudo = w1 * pred_pseudo + w2 * shapelet_pseudo
    return pseudo
